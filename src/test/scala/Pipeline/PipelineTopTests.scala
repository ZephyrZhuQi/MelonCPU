...
class PipelineTopTests(c: Top) extends PeekPokeTester(c) {
  ...
  val file        = new File("InstFiles/Test")
  var inst_amount = 0
  var cycles      = 0
  var IPC         = 0
  var inst_last   = 0
  ...
  def RunCpu (defcycle : Int, filename : File, test_num : Int) =
  {
    ...
    while(peek(c.io.MEM_WB_inst)!=0x26730001 && cycles<21000/**peek(c.io.MEM_WB_inst) != (inst_last)*/)
    {
...
object PipelineTopTests {
  def main (args: Array[String]): Unit = {
    var backEnd = "verilator"
    if (args.nonEmpty){
      backEnd = args(0)
    }
//    iotesters.Driver.execute(args,()=>new Top.Top){
//      c => new PiplineTopTests(c)
//    }

    Driver(()=>new Top,backEnd){
